         ...
         <div className="flex flex-1 flex-col gap-14">
          <div className="flex flex-col gap-10">
            {paginatedNews?.map((article, index) => (
              // dynamic Tailwind class strings like `[animation-delay:${index*150}ms]` never get
              // generated by Tailwind's compiler since it scans source text literally, so this
              // uses an inline style instead, which always works regardless of the value
              <div
              ...
              >
                ...
                <Link
                  to={`/news/${article._id}`}
                  className="mt-5 inline-flex items-center gap-2 rounded-full bg-[#dde9fc] px-6 py-3 font-semibold text-[#161654] transition-colors duration-300 hover:bg-[#3EA6E0] hover:text-white"
                >
        ...
        <div className="flex w-full flex-col gap-8 lg:w-[320px]">
          ...
          {newsData?.slice(0, 3).map((article) => (
            <Link
              to={`/news/${article._id}`}
              key={article._id}
              className="flex gap-3 border-b border-gray-100 pb-4 last:border-0 transition-opacity hover:opacity-70"
            >
              <div className="h-14 w-14 shrink-0 rounded-lg bg-gray-200 overflow-hidden">
                {article.image && (
                  <img src={article.image} alt={article.title} className="h-full w-full object-cover" />
                )}
              </div>
